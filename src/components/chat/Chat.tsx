...
type ChatHistory = {
  ...
  useEffect(() => {
    if (user) {
      fetchChatHistories(user.id).then(({ data }) => {
        if (data) setChatHistories((data as any[]).map((c: any) => ({ ...c, messages: c.messages || [] })));
      });
    } else {
      setChatHistories([]);
    }
    ...
    try {
      ...
      if (user) {
        if (activeChatId) {
          ...
          // 取得新 id
          if (data && data[0]?.id) setActiveChatId(data[0].id);
        }
        // 重新取得歷史紀錄
        fetchChatHistories(user.id).then(({ data }) => {
          if (data) setChatHistories((data as any[]).map((c: any) => ({ ...c, messages: c.messages || [] })));
  ...
  return (
    <div className="min-h-screen bg-[#18132a] flex text-white relative">
      <ChatTopbar user={user} onLogout={async () => { await supabase.auth.signOut(); }} />
      <ChatSidebar
        user={user}
